...
//            f= ps.executeUpdate() > 0;
    ...
    @Override
    public List<ArtDetails> getArtBySearch(String ch) {
        List<ArtDetails> list = new ArrayList<ArtDetails>();
        ArtDetails ad = null;
        try {
            String sql = "select * from  art_details where artName like ? or artistName like ? or artCategory like ? and status=?";
            PreparedStatement ps = conn.prepareStatement(sql);
